...
-- Recriar a versão TEXT da RPC get_vendas_por_pedido com lógica correta:
-- Flex/Flex Turbo: frete_efetivo = custo_config - bonus_envio (subsídio abate custo operacional)
-- Full/Coleta/Outros: frete_efetivo = frete_vendedor (valor real da API, bonus_envio ignorado)
...
BEGIN
  ...
  -- Converter datas texto -> timestamptz (BR UTC-3)
  ...
  WITH vendas_base AS (
    SELECT
      COALESCE(mt.pack_id, mt.pedido_id)            AS grp_id,
      mt.empresa_id,
      e.nome_fantasia,
      mt.canal,
      mt.conta_nome,
      ...
      CASE WHEN bool_and(mt.frete_vendedor IS NOT NULL) THEN SUM(COALESCE(mt.frete_vendedor, 0)) ELSE NULL END AS frete_vendedor_api,
      SUM(COALESCE(mt.custo_ads, 0))                 AS ads_total,
      SUM(COALESCE(mt.rebate, 0))                    AS rebate_agg,
      -- bonus_envio: relevante SOMENTE para flex/flex_turbo
      CASE
        WHEN MAX(mt.tipo_envio) IN ('flex', 'flex_turbo')
        THEN SUM(COALESCE(mt.bonus_envio, 0))
        ELSE 0
      END                                            AS bonus_envio_agg,
  ...
  itens_agg AS (
    SELECT
      COALESCE(mt2.pack_id, mt2.pedido_id)                                         AS grp_id,
      mt2.empresa_id,
  ...
  resultado AS (
    SELECT
      ...
      -- frete_vendedor_total exibido na UI:
      -- Flex/Turbo: custo_config - bonus (custo real pago à transportadora)
      -- Full/Coleta/Outros: frete real da API
      CASE
        ...
        ELSE COALESCE(vb.frete_vendedor_api, 0)
      ...
      ROUND(
        ...
        - CASE
            WHEN vb.tipo_envio IN ('flex', 'flex_turbo') THEN
              GREATEST(0,
                COALESCE((SELECT lpc2.custo FROM logistica_plataforma_config lpc2
                  WHERE lpc2.empresa_id = vb.empresa_id AND lpc2.canal = vb.canal AND lpc2.tipo_envio = vb.tipo_envio LIMIT 1), 0)
                - vb.bonus_envio_agg
              )
            ELSE COALESCE(vb.frete_vendedor_api, 0)
          END
      ...
      ROUND(
        ...
        - CASE
            WHEN vb.tipo_envio IN ('flex', 'flex_turbo') THEN
              GREATEST(0,
                COALESCE((SELECT lpc3.custo FROM logistica_plataforma_config lpc3
                  WHERE lpc3.empresa_id = vb.empresa_id AND lpc3.canal = vb.canal AND lpc3.tipo_envio = vb.tipo_envio LIMIT 1), 0)
                - vb.bonus_envio_agg
              )
            ELSE COALESCE(vb.frete_vendedor_api, 0)
          END
        ...
        WHEN vb.valor_produto > 0 AND ia.cmv_total IS NOT NULL THEN
          ROUND((
            ...
            - CASE
                WHEN vb.tipo_envio IN ('flex', 'flex_turbo') THEN
                  GREATEST(0,
                    COALESCE((SELECT lpc4.custo FROM logistica_plataforma_config lpc4
                      WHERE lpc4.empresa_id = vb.empresa_id AND lpc4.canal = vb.canal AND lpc4.tipo_envio = vb.tipo_envio LIMIT 1), 0)
                    - vb.bonus_envio_agg
                  )
                ELSE COALESCE(vb.frete_vendedor_api, 0)
              END
    ...
    LEFT JOIN itens_agg    ia ON ia.grp_id = vb.grp_id AND ia.empresa_id = vb.empresa_id
  )
  SELECT r.*
  FROM resultado r
  WHERE
