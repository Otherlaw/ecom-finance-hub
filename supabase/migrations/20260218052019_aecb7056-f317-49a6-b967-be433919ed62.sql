...
-- Corrigir RPC get_vendas_por_pedido:
-- Restaurar hierarquia de custo: produto.custo_medio > sku_costs.custo_unitario
...
BEGIN
  ...
  itens_agg AS (
    SELECT
      ...
      -- Hierarquia de custo: produto.custo_medio > sku_costs.custo_unitario
      CASE
        WHEN bool_and(
          COALESCE(p.custo_medio, 0) > 0 OR COALESCE(sc.custo_unitario, 0) > 0
        )
        THEN SUM(
  ...
  resultado AS (
    SELECT
      ...
      ROUND(
        ...
        - CASE
            WHEN vb.tipo_envio IN ('flex', 'self_service') THEN COALESCE(lc.flex_custo, 0)
            WHEN vb.tipo_envio = 'flex_turbo' THEN COALESCE(lc.flex_turbo_custo, 0)
            ELSE 0
          END
      , 2) AS valor_liquido,
      ia.cmv_total,
    ...
    LEFT JOIN logistica_config lc ON lc.empresa_id = vb.empresa_id
