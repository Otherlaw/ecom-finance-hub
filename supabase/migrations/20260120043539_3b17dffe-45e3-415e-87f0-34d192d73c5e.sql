-- Atualizar RPCs para considerar tarifa_fixa como tipo_evento separado
-- e manter compatibilidade com tarifa_financeira

-- ============================================================
-- RPC: get_vendas_por_pedido (com separação de tarifa_fixa e tarifa_financeira)
-- ============================================================
...
BEGIN
  ...
  WITH pedidos_base AS (
    SELECT
      ...
      -- Legado: taxas = comissao + tarifa_fixa, tarifas = tarifa_financeira
  ...
  eventos_por_pedido AS (
    SELECT
      fe.pedido_id,
      -- Comissão separada (tipo_evento = 'comissao')
      COALESCE(SUM(CASE WHEN fe.tipo_evento = 'comissao' THEN ABS(fe.valor) ELSE 0 END), 0) AS comissao_eventos,
      -- Tarifa fixa + tarifa financeira combinadas para UI (campo tarifa_fixa_total)
      ...
      -- Outros (estornos, cancelamentos, ajustes)
  ...
  SELECT
    ...
    pb.tipo_envio,
    pb.valor_produto,
    -- Comissão: priorizar eventos, fallback para legado (taxas)
    CASE WHEN COALESCE(ep.total_eventos, 0) > 0 THEN ep.comissao_eventos ELSE pb.taxas_legado END AS comissao_total,
    -- Tarifa Fixa: priorizar eventos (tarifa_fixa + tarifa_financeira), fallback para legado (tarifas)
    ...
    -- Impostos (ainda não implementado, sempre 0)
    ...
    -- Valor líquido: valor_produto - comissao - tarifa
    (pb.valor_produto - 
    ...
    -- CMV: NULL se nenhum item tem custo válido
    CASE 
      WHEN COALESCE(cp.itens_com_custo, 0) = 0 THEN NULL
      ELSE cp.cmv_calculado
    END AS cmv_total,
    -- Margem de contribuição: NULL se CMV é NULL
    CASE 
      WHEN COALESCE(cp.itens_com_custo, 0) = 0 THEN NULL
      ELSE (
        pb.valor_produto 
...
-- ============================================================
-- RPC: get_vendas_por_pedido_resumo (atualizar para mesma lógica)
-- ============================================================
...
BEGIN
  ...
  cmv_por_pedido AS (
    SELECT
      mt.pedido_id,
      COALESCE(SUM(mti.quantidade), 0) AS qtd_itens,
      COUNT(CASE WHEN p.custo_medio IS NOT NULL AND p.custo_medio > 0 THEN 1 END) AS itens_com_custo,
      SUM(mti.quantidade * p.custo_medio) AS cmv_calculado
    FROM marketplace_transactions mt
    LEFT JOIN marketplace_transaction_items mti ON mti.transaction_id = mt.id
  ...
  pedidos_calculados AS (
    SELECT
      ...
      CASE WHEN COALESCE(ep.total_eventos, 0) > 0 THEN ep.outros_eventos ELSE pb.outros_descontos_legado END AS outros,
      cp.qtd_itens,
      CASE WHEN COALESCE(cp.itens_com_custo, 0) = 0 THEN NULL ELSE cp.cmv_calculado END AS cmv
    FROM pedidos_base pb
    LEFT JOIN eventos_por_pedido ep ON ep.pedido_id = pb.pedido_id
    LEFT JOIN cmv_por_pedido cp ON cp.pedido_id = pb.pedido_id
  )
  SELECT
    ...
    COALESCE(SUM(pc.valor_produto), 0) AS valor_produto_total,
    COALESCE(SUM(pc.comissao), 0) AS comissao_total,
    COALESCE(SUM(pc.tarifa), 0) AS tarifa_fixa_total,
    COALESCE(SUM(pc.frete_vendedor), 0) AS frete_vendedor_total,
    COALESCE(SUM(pc.ads), 0) AS ads_total,
    0::NUMERIC AS impostos_total,
    COALESCE(SUM(pc.outros), 0) AS outros_descontos_total,
    COALESCE(SUM(pc.valor_produto - pc.comissao - pc.tarifa), 0) AS valor_liquido_total,
    COALESCE(SUM(pc.qtd_itens), 0)::BIGINT AS qtd_itens_total,
    SUM(pc.cmv) AS cmv_total,
    SUM(
      CASE WHEN pc.cmv IS NULL THEN NULL
      ELSE pc.valor_produto - pc.comissao - pc.tarifa - pc.frete_vendedor - pc.ads - COALESCE(pc.cmv, 0)
      END
    ) AS margem_contribuicao_total
  FROM pedidos_calculados pc;
END;
$$;