-- =============================================================================
-- Corrigir CMV nas RPCs de vendas por pedido:
-- - Retornar NULL quando não há custo (não 0 falso)
-- - Calcular margem de contribuição corretamente
-- - Marcar pedidos sem CMV para fácil identificação
-- =============================================================================

-- Dropar funções existentes para poder recriar com nova assinatura
...
-- Recriar função get_vendas_por_pedido com CMV correto
...
BEGIN
  ...
  cmv_por_pedido AS (
    SELECT
      ...
      -- Total de itens que têm custo cadastrado
      ...
      -- CMV: SUM de quantidade * custo_medio (apenas para itens com custo)
  ...
  SELECT
    ...
    -- CMV: NULL se nenhum item tem custo válido, senão o valor calculado
    CASE 
    ...
    -- Flag para indicar se tem CMV calculável
...
-- =============================================================================
-- Atualizar RPC de resumo para também retornar CMV corretamente
-- =============================================================================
...
BEGIN
  ...
  SELECT
    ...
    -- CMV total (soma apenas dos pedidos com CMV)
    COALESCE(SUM(CASE WHEN pc.tem_cmv THEN pc.cmv_calculado ELSE 0 END), 0) AS cmv_total,
    -- Margem total (soma apenas dos pedidos com CMV)
    COALESCE(SUM(
      CASE WHEN pc.tem_cmv THEN
        pc.valor_produto - pc.comissao - pc.tarifa - pc.frete_vendedor - pc.ads - COALESCE(pc.cmv_calculado, 0)
      ELSE 0 END
...
-- Garantir permissões
GRANT EXECUTE ON FUNCTION public.get_vendas_por_pedido TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_vendas_por_pedido_resumo TO authenticated;