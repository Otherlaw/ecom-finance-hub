...
-- Drop and recreate get_vendas_por_pedido with additional filter parameters
DROP FUNCTION IF EXISTS get_vendas_por_pedido(uuid, date, date, text, text, text, text, int, int);
...
CREATE OR REPLACE FUNCTION get_vendas_por_pedido(
  ...
  p_tem_custo TEXT DEFAULT NULL  -- 'com_custo', 'sem_custo', or NULL for all
)
RETURNS TABLE (
  pedido_id TEXT,
  empresa_id UUID,
...
BEGIN
  ...
  itens_agregados AS (
    SELECT
      ...
      -- CMV de itens físicos (quando existem)
      SUM(
        CASE
          ...
          WHEN mti.sku_marketplace IS NOT NULL THEN
            COALESCE(
              ...
              (SELECT sc.custo_unitario FROM sku_costs sc WHERE sc.sku_marketplace = mti.sku_marketplace AND sc.empresa_id = mt.empresa_id LIMIT 1),
              0
            ) * mti.quantidade
          ELSE 0
        END
  ...
  resultado_final AS (
    SELECT
      ...
      ia.qtd_itens,
      -- CMV: prioriza itens físicos, fallback para raw_order
      CASE 
  ...
  SELECT
    ...
    rf.qtd_itens,
    rf.cmv_total,
    -- Margem de contribuição: só calcula se CMV existir
    CASE 
  ...
  -- Apply cost filter at the final stage
...
-- Also update the count function to include the new filters
DROP FUNCTION IF EXISTS get_vendas_por_pedido_count(uuid, date, date, text, text, text, text);

CREATE OR REPLACE FUNCTION get_vendas_por_pedido_count(
  p_empresa_id UUID DEFAULT NULL,
  p_data_inicio DATE DEFAULT NULL,
  p_data_fim DATE DEFAULT NULL,
  p_canal TEXT DEFAULT NULL,
...
BEGIN
  ...
  -- For 'tem_custo' filter, we need a more complex query
            ...
            CASE
              ...
              WHEN mti.sku_marketplace IS NOT NULL THEN
                COALESCE(
                  ...
                  (SELECT sc.custo_unitario FROM sku_costs sc WHERE sc.sku_marketplace = mti.sku_marketplace AND sc.empresa_id = pb.empresa_id LIMIT 1),
                  0
                ) * mti.quantidade
              ELSE 0
            END
