...
-- Recriar get_vendas_por_pedido com nomes de colunas corretos
-- Colunas reais: valor_bruto, tarifas (comissão), taxas (tarifa_fixa), frete_vendedor, custo_ads, bonus_envio
-- Itens: transaction_id, produto_id, sku_marketplace, anuncio_id, quantidade, preco_total

DROP FUNCTION IF EXISTS public.get_vendas_por_pedido(uuid, text, text, text, text, text, text, text, text, integer, integer);
DROP FUNCTION IF EXISTS public.get_vendas_por_pedido(uuid, text, text, text, text, text, text, text, text, text, text);
...
RETURNS TABLE (
  ...
  status                  text,
  tipo_envio              text,
  valor_produto           numeric,   -- alias para valor_bruto
  comissao_total          numeric,   -- tarifas (CV)
  tarifa_fixa_total       numeric,   -- taxas (FINANCING_FEE etc)
...
BEGIN
  ...
  RETURN QUERY
  -- 1. Identificar os group_keys da página (paginação antecipada)
  ...
  -- 2. Agregar transações apenas dos pedidos da página
  pedidos_base AS (
    SELECT
      COALESCE(mt.pack_id, mt.pedido_id, mt.referencia_externa) AS grp_id,
      mt.empresa_id,
      ...
      -- valor_bruto é a receita bruta (valor do produto)
      SUM(mt.valor_bruto)           AS valor_bruto_agg,
      -- tarifas = comissão de venda (CV)
      SUM(COALESCE(mt.tarifas, 0))  AS comissao_agg,
      -- taxas = tarifa fixa (FINANCING_FEE, etc)
      SUM(COALESCE(mt.taxas, 0))    AS tarifa_fixa_agg,
      -- frete_vendedor direto da API
      SUM(COALESCE(mt.frete_vendedor, 0)) AS frete_vendedor_api,
      -- bonus_envio = desconto de frete recebido pelo vendedor
  ...
  resultado AS (
    SELECT
      ...
      pb.comissao_agg,
      pb.tarifa_fixa_agg,
      -- Frete vendedor com lógica Flex corrigida
      CASE
