-- Corrigir tipos de retorno no CASE para tem_cmv
...
BEGIN
  ...
  resultado_final AS (
    SELECT
      ...
      CASE 
        WHEN ia.item_count > 0 THEN COALESCE(ia.cmv_itens, 0)
        ELSE COALESCE(rc.cmv_raw, 0)
      END AS cmv_total,
  ...
  SELECT
    ...
    rf.ads_total,
    ROUND(rf.valor_produto * v_aliquota_imposto / 100, 2) AS impostos_total,
