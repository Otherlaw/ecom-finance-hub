-- Forçar cast para numeric em cmv_total
...
BEGIN
  ...
  resultado_final AS (
    SELECT
      ...
      ia.qtd_itens,
      -- CMV: prioriza itens físicos, senão usa raw_cmv
      CASE 
        WHEN ia.item_count > 0 THEN COALESCE(ia.cmv_itens, 0::numeric)
        ELSE COALESCE(rc.cmv_raw, 0::numeric)
      END AS cmv_total,
      -- tem_cmv: prioriza itens físicos, senão verifica raw_cmv
      CASE 
        WHEN ia.item_count > 0 THEN COALESCE(ia.tem_cmv_itens, FALSE)
  ...
  SELECT
    ...
    rf.ads_total,
    ROUND(rf.valor_produto * v_aliquota_imposto / 100, 2)::numeric AS impostos_total,
    ...
    ROUND(
      ...
      - rf.ads_total 
      - rf.outros_descontos_total 
      - (rf.valor_produto * v_aliquota_imposto / 100),
      2
    )::numeric AS valor_liquido_calculado,
    rf.qtd_itens,
    rf.cmv_total::numeric,
    ROUND(
      rf.valor_produto 
      - rf.comissao_total 
      - rf.tarifa_fixa_total 
      ...
      - rf.cmv_total,
      2
    )::numeric AS margem_contribuicao,
    rf.tem_cmv
