...
BEGIN
  ...
  WITH vendas_base AS (
    SELECT
      ...
      -- bonus_envio SOMENTE para flex/flex_turbo
      CASE
        WHEN MAX(mt.tipo_envio) IN ('flex', 'flex_turbo') THEN SUM(COALESCE(mt.bonus_envio, 0))
        ELSE 0
  ...
  resultado AS (
    SELECT
      ...
      -- frete_vendedor_total: flex usa config-bonus, outros usam valor real
      CASE
        WHEN vb.tipo_envio IN ('flex', 'flex_turbo') THEN
          GREATEST(0,
            COALESCE((SELECT lpc.custo FROM logistica_plataforma_config lpc
              WHERE lpc.empresa_id = vb.empresa_id AND lpc.canal = vb.canal AND lpc.tipo_envio = vb.tipo_envio LIMIT 1), 0)
            - COALESCE(vb.bonus_envio_agg, 0)
          )
        ELSE vb.frete_vendedor_agg
      ...
      ROUND(
        ...
        - CASE
            WHEN vb.tipo_envio IN ('flex', 'flex_turbo') THEN
              GREATEST(0,
                COALESCE((SELECT lpc.custo FROM logistica_plataforma_config lpc WHERE lpc.empresa_id = vb.empresa_id AND lpc.canal = vb.canal AND lpc.tipo_envio = vb.tipo_envio LIMIT 1), 0)
                - COALESCE(vb.bonus_envio_agg, 0)
              )
            ELSE COALESCE(vb.frete_vendedor_agg, 0)
          END
