...
import { CalendarCheck, RefreshCw, Receipt, BarChart3, Building2, TrendingUp, ArrowRight, CheckCircle2, Shield, Zap } from "lucide-react";
export default function Landing() {
  ...
  const marketplaces = ["Mercado Livre", "Shopee", "Amazon", "Magalu", "Americanas", "TikTok Shop"];
  return <div className="min-h-screen bg-background">
      {/* Header */}
      <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img alt="ECOM Finance" className="h-10" src="/lovable-uploads/4dc95883-4a13-4da4-8c62-f571be298071.png" />
          </div>
      ...
      <section className="py-12 border-y bg-muted/30">
        <div className="container mx-auto px-4">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 md:gap-8">
            {marketplaces.map(marketplace => <div key={marketplace} className="px-4 py-2 rounded-lg bg-background border text-sm font-medium text-muted-foreground">
                {marketplace}
              </div>)}
          </div>
        </div>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {steps.map((step, index) => <div key={step.number} className="relative">
                {index < steps.length - 1 && <div className="hidden md:block absolute top-8 left-1/2 w-full h-px bg-border" />}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <ul className="space-y-3">
                  {["Criptografia SSL/TLS em todas as conexões", "Dados armazenados com segurança em nuvem", "Acesso controlado por autenticação segura", "Backups automáticos diários"].map(item => <li key={item} className="flex items-center gap-2 text-sm">
      ...
      <section className="py-20 md:py-28 bg-primary text-primary-foreground">
        <div className="container mx-auto px-4 text-center">
          ...
          <Button size="lg" variant="secondary" onClick={() => navigate("/auth")} className="gap-2 h-12 px-8 text-base">
            Acessar agora
            <ArrowRight className="h-5 w-5" />
          </Button>
        </div>
