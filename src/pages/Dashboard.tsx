...
const CHART_COLORS = ["hsl(4, 86%, 55%)", "hsl(217, 91%, 60%)", "hsl(142, 71%, 45%)", "hsl(38, 92%, 50%)", "hsl(262, 83%, 58%)", "hsl(190, 95%, 39%)"];
...
export default function Dashboard() {
  ...
  const channelData = useMemo(() => {
    const CHART_COLORS_LOCAL = ["hsl(4, 86%, 55%)", "hsl(217, 91%, 60%)", "hsl(142, 71%, 45%)", "hsl(38, 92%, 50%)", "hsl(262, 83%, 58%)", "hsl(190, 95%, 39%)"];
    const total = mktChannelData.reduce((a, c) => a + c.valor, 0);
    return mktChannelData.map((c, index) => ({
      channel: c.channel,
      valor: c.valor,
  ...
  return <MainLayout title="Dashboard Executivo" subtitle="Visão geral consolidada do seu e-commerce" actions={<div className="flex items-center gap-3 flex-wrap">
      ...
      {isLoading ? <div className="flex items-center justify-center py-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <KPICard title="Faturamento Bruto" value={formatCurrency(kpis.receitaBruta)} icon={DollarSign} iconColor="text-primary" trend="neutral" />
            <KPICard title="Lucro Líquido" value={formatCurrency(kpis.lucroLiquido)} icon={TrendingUp} iconColor={kpis.lucroLiquido >= 0 ? "text-success" : "text-destructive"} trend={kpis.lucroLiquido >= 0 ? "up" : "down"} />
            <KPICard title="Margem Bruta" value={`${kpis.margemBruta.toFixed(1)}%`} icon={Percent} iconColor="text-info" trend={kpis.margemBruta >= 20 ? "up" : "down"} />
            <KPICard title="Pedidos" value={formatNumber(kpis.pedidos)} icon={ShoppingCart} iconColor="text-warning" trend="neutral" />
          </div>

          {/* Secondary KPIs */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <KPICard title="Ticket Médio" value={formatCurrency(kpis.ticketMedio)} icon={CreditCard} trend="neutral" />
            ...
            <KPICard title="Margem Líquida" value={`${kpis.margemLiquida.toFixed(1)}%`} icon={Percent} trend={kpis.margemLiquida >= 0 ? "up" : "down"} />
            <KPICard title="Despesas Operacionais" value={formatCurrency(kpis.totalDespesas)} changeLabel={`${kpis.despesasPercentual.toFixed(1)}% da receita`} icon={BarChart3} trend="neutral" />
          </div>

          {/* Charts Row */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            {/* Cash Flow */}
            <ModuleCard title="Fluxo de Caixa" description="Últimos 30 dias" icon={BarChart3}>
              {cashFlowData.length > 0 ? <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={cashFlowData}>
                      <CartesianGrid strokeDasharray="3 3" stroke="hsl(220, 13%, 91%)" />
                      <XAxis dataKey="data" stroke="hsl(220, 10%, 46%)" fontSize={10} />
                      <YAxis stroke="hsl(220, 10%, 46%)" fontSize={10} tickFormatter={value => `${(value / 1000).toFixed(0)}k`} />
            ...
            {/* Revenue by Channel */}
            <ModuleCard title="Receita por Canal" description="Distribuição de vendas" icon={PieChart}>
              {channelData.length > 0 ? <div className="h-[300px] flex items-center">
                  <ResponsiveContainer width="100%" height="100%">
                    <RechartsPieChart>
                      <Pie data={channelData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={4} dataKey="valor" nameKey="channel">
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {topProdutosProcessados.map(produto => <TableRow key={produto.id}>
                          ...
                          <TableCell className="text-right">
                            <Badge variant="outline" className={produto.margem >= 20 ? "bg-success/10 text-success border-success/30" : produto.margem >= 10 ? "bg-warning/10 text-warning border-warning/30" : "bg-destructive/10 text-destructive border-destructive/30"}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <ModuleCard title="Maiores Despesas" description="Por categoria" icon={CreditCard}>
              {expensesByCategory.length > 0 ? <div className="space-y-4">
                  {expensesByCategory.map((expense, index) => <div key={expense.category} className="space-y-2">
          ...
          <div className="mt-6">
            <ModuleCard title="Saúde Financeira" description="Indicadores críticos do negócio com análise detalhada">
              ...
              return <TooltipProvider key={indicador.id}>
                      <TooltipUI>
                        <TooltipTrigger asChild>
                          <div className={`p-4 rounded-xl border cursor-help transition-all hover:shadow-md ${statusStyles[status]}`}>
                            ...
                            <div className="flex items-center justify-between">
                              <Badge variant="outline" className={`text-xs ${status === "saudavel" ? "border-success/30 text-success" : status === "alerta" ? "border-warning/30 text-warning" : status === "critico" ? "border-destructive/30 text-destructive" : "border-border"}`}>
                                {statusLabels[status]}
                              </Badge>
