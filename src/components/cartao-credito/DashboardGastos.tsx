...
import { AlertCircle, TrendingUp, CreditCard, Tags, Building2, Calendar, Filter } from "lucide-react";
import { useCartoesDashboard } from "@/hooks/useCartoesDashboard";
...
export function DashboardGastos() {
  ...
  const [periodoSelecionado, setPeriodoSelecionado] = useState<string>(format(new Date(), "yyyy-MM"));

  // Buscar lista de cartões para o select
  const { cartoes: listaCartoes, isLoading: isLoadingCartoes } = useCartoes();

  // Buscar dados do dashboard com filtros
  const { transacoes, kpis, isLoading } = useCartoesDashboard({
    cartaoId: cartaoSelecionado === "todos" ? undefined : cartaoSelecionado,
  ...
  // Encontrar nome do cartão selecionado para exibição
  const nomeCartaoSelecionado = useMemo(() => {
    if (cartaoSelecionado === "todos") return "Todos os cartões";
    const cartao = listaCartoes?.find((c: any) => c.id === cartaoSelecionado);
    return cartao?.nome || "Cartão";
  }, [cartaoSelecionado, listaCartoes]);

  // Encontrar label do período selecionado
  const labelPeriodoSelecionado = useMemo(() => {
    const opcao = opcoesPeriodo.find((o) => o.value === periodoSelecionado);
    return opcao?.label || periodoSelecionado;
  }, [periodoSelecionado, opcoesPeriodo]);

  if (isLoading || isLoadingCartoes) {
    return (
        ...
        <CardContent className="pt-6">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="ml-auto text-sm text-muted-foreground">
              <span className="font-medium">{nomeCartaoSelecionado}</span>
              {" • "}
          ...
          <CardContent className="py-12">
            <div className="flex flex-col items-center justify-center text-center">
              ...
              <p className="text-sm text-muted-foreground mb-4 max-w-md">
                Não há transações para{" "}
                <span className="font-medium">{nomeCartaoSelecionado.toLowerCase()}</span> em{" "}
                <span className="font-medium">{labelPeriodoSelecionado.toLowerCase()}</span>.
              </p>
              <p className="text-xs text-muted-foreground">
                Tente selecionar outro período ou cartão, ou importe novas faturas.
              </p>
            </div>
          </CardContent>
        </Card>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">{kpis.qtdTransacoes} transações</p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {transacoes.length > 0 
                        ? `${((totais.categorizadas / transacoes.length) * 100).toFixed(0)}% do total`
                        : "0% do total"}
                    </p>
                  </div>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {transacoes.length > 0 
                        ? `${((totais.pendentes / transacoes.length) * 100).toFixed(0)}% do total`
                        : "0% do total"}
                    </p>
                  </div>
