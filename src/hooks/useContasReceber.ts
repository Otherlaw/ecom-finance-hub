import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";
import { registrarMovimentoFinanceiro, excluirMovimentoPorReferencia } from "@/hooks/useMovimentosFinanceiros";

export interface ContaReceber {
  id: string;
  empresa_id: string;
  cliente_nome: string;
  descricao: string;
  documento: string | null;
  tipo_lancamento: string;
  origem: string | null;
  data_emissao: string;
  data_vencimento: string;
  data_recebimento: string | null;
  valor_total: number;
  valor_recebido: number;
  valor_em_aberto: number;
  status: string;
  categoria_id: string | null;
  centro_custo_id: string | null;
  forma_recebimento: string | null;
  observacoes: string | null;
  recorrente: boolean;
  conciliado: boolean;
  created_at: string;
  updated_at: string;
  // Joins
  empresa?: { id: string; razao_social: string; nome_fantasia: string | null };
  categoria?: { id: string; nome: string; tipo: string };
  centro_custo?: { id: string; nome: string };
}

export type StatusContaReceber = "em_aberto" | "parcialmente_recebido" | "recebido" | "vencido" | "cancelado";

export const STATUS_LABELS_RECEBER: Record<string, { label: string; color: string }> = {
  em_aberto: { label: "Em Aberto", color: "bg-blue-100 text-blue-800" },
  parcialmente_recebido: { label: "Parcial", color: "bg-amber-100 text-amber-800" },
  recebido: { label: "Recebido", color: "bg-green-100 text-green-800" },
  vencido: { label: "Vencido", color: "bg-red-100 text-red-800" },
  cancelado: { label: "Cancelado", color: "bg-gray-100 text-gray-800" },
};

interface UseContasReceberParams {
  empresaId?: string;
  status?: string;
  dataInicio?: string;
  dataFim?: string;
}

export const useContasReceber = (params: UseContasReceberParams = {}) => {
  const queryClient = useQueryClient();
  const { empresaId, status, dataInicio, dataFim } = params;

  const { data: contas, isLoading, refetch } = useQuery({
    queryKey: ["contas-a-receber", empresaId, status, dataInicio, dataFim],
    queryFn: async () => {
      let query = supabase
        .from("contas_a_receber")
        .select(`
          *,
          empresa:empresas(id, razao_social, nome_fantasia),
          categoria:categorias_financeiras(id, nome, tipo),
          centro_custo:centros_de_custo(id, nome)
        `)
        .order("data_vencimento", { ascending: true });

      if (empresaId && empresaId !== "todas") {
        query = query.eq("empresa_id", empresaId);
      }

      if (status && status !== "todos") {
        query = query.eq("status", status);
      }

      if (dataInicio) {
        query = query.gte("data_vencimento", dataInicio);
      }

      if (dataFim) {
        query = query.lte("data_vencimento", dataFim);
      }

      const { data, error } = await query;
      if (error) throw error;
      return (data || []) as ContaReceber[];
    },
  });

  const createConta = useMutation({
    mutationFn: async (conta: Omit<ContaReceber, "id" | "created_at" | "updated_at" | "empresa" | "categoria" | "centro_custo">) => {
      const { data, error } = await supabase
        .from("contas_a_receber")
        .insert(conta)
        .select()
        .single();

      if (error) throw error;
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["contas-a-receber"] });
      toast.success("Conta a receber cadastrada com sucesso!");
    },
    onError: (error: any) => {
      toast.error("Erro ao cadastrar: " + error.message);
    },
  });

  const updateConta = useMutation({
    mutationFn: async ({ id, ...conta }: Partial<ContaReceber> & { id: string }) => {
      const { data, error } = await supabase
        .from("contas_a_receber")
        .update(conta)
        .eq("id", id)
        .select()
        .single();

      if (error) throw error;
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["contas-a-receber"] });
      toast.success("Conta atualizada com sucesso!");
    },
    onError: (error: any) => {
      toast.error("Erro ao atualizar: " + error.message);
    },
  });

  const deleteConta = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase
        .from("contas_a_receber")
        .delete()
        .eq("id", id);

      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["contas-a-receber"] });
      toast.success("Conta excluída com sucesso!");
    },
    onError: (error: any) => {
      toast.error("Erro ao excluir: " + error.message);
    },
  });

  const receberConta = useMutation({
    mutationFn: async ({ id, valorRecebido, dataRecebimento }: { id: string; valorRecebido: number; dataRecebimento: string }) => {
      // Buscar conta atual com dados completos
      const { data: contaAtual, error: fetchError } = await supabase
        .from("contas_a_receber")
        .select(`
          *,
          categoria:categorias_financeiras(id, nome),
          centro_custo:centros_de_custo(id, nome)
        `)
        .eq("id", id)
        .single();

      if (fetchError) throw fetchError;

      const novoValorRecebido = (contaAtual?.valor_recebido || 0) + valorRecebido;
      const novoValorEmAberto = (contaAtual?.valor_total || 0) - novoValorRecebido;
      const novoStatus = novoValorEmAberto <= 0 ? "recebido" : "parcialmente_recebido";

      const { data, error } = await supabase
        .from("contas_a_receber")
        .update({
          valor_recebido: novoValorRecebido,
          valor_em_aberto: Math.max(0, novoValorEmAberto),
          status: novoStatus,
          data_recebimento: novoStatus === "recebido" ? dataRecebimento : null,
        })
        .eq("id", id)
        .select()
        .single();

      if (error) throw error;

      // Registrar movimento no MEU
      await registrarMovimentoFinanceiro({
        data: dataRecebimento,
        tipo: "entrada",
        origem: "contas_receber",
        descricao: contaAtual.descricao,
        valor: novoValorRecebido, // Valor total recebido consolidado
        empresa_id: contaAtual.empresa_id,
        referencia_id: id,
        categoria_id: contaAtual.categoria_id || undefined,
        categoria_nome: (contaAtual.categoria as any)?.nome || undefined,
        centro_custo_id: contaAtual.centro_custo_id || undefined,
        centro_custo_nome: (contaAtual.centro_custo as any)?.nome || undefined,
        forma_pagamento: contaAtual.forma_recebimento || undefined,
        cliente_nome: contaAtual.cliente_nome,
        observacoes: "Recebimento de título a receber",
      });

      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["contas-a-receber"] });
      queryClient.invalidateQueries({ queryKey: ["movimentos_financeiros"] });
      queryClient.invalidateQueries({ queryKey: ["dre-contas-receber"] });
      toast.success("Recebimento registrado com sucesso!");
    },
    onError: (error: any) => {
      toast.error("Erro ao registrar recebimento: " + error.message);
    },
  });

  // Calcular resumo
  const resumo = {
    totalEmAberto: contas?.filter(c => c.status !== "recebido" && c.status !== "cancelado").reduce((acc, c) => acc + c.valor_em_aberto, 0) || 0,
    totalVencido: contas?.filter(c => c.status === "vencido" || (c.status === "em_aberto" && new Date(c.data_vencimento) < new Date())).reduce((acc, c) => acc + c.valor_em_aberto, 0) || 0,
    totalRecebido: contas?.filter(c => c.status === "recebido").reduce((acc, c) => acc + c.valor_total, 0) || 0,
    quantidade: contas?.length || 0,
  };

  return {
    contas,
    isLoading,
    refetch,
    createConta,
    updateConta,
    deleteConta,
    receberConta,
    resumo,
  };
};
